    ...
    <div>
      <ul className="space-y-1.5">
        {visiveis.map((d) => {
          ...
          return (
            <li
              key={d.chave}
              className={`overflow-hidden rounded-lg border bg-slate-950/35 backdrop-blur-xl ${
                d.ehHoje ? "border-[#c69e6b]/70 ring-1 ring-[#c69e6b]/40" : "border-white/20"
              }`}
            >
              <button
                type="button"
                ...
                className="flex w-full items-center gap-3 px-3.5 py-2.5 text-left active:bg-white/[0.06]"
              >
                <span className="w-[52px] shrink-0 text-[15px] font-bold tabular-nums text-white">
                  {d.diaMes}
                </span>

                <span className="w-[34px] shrink-0 text-[11px] uppercase text-white/50">
                ...
                <span
                  className={`w-[62px] shrink-0 text-right text-[13px] font-bold tabular-nums ${
                    d.saldoMinutos < 0 ? "text-rose-300" : "text-teal-200"
                  }`}
                >
                  {d.saldoTexto}
                </span>
              ...
              {aberto && (
                <div className="border-t border-white/10 px-3.5 pb-3 pt-2.5">
                  <div className="flex flex-wrap gap-1.5">
                    {d.batidas.map((b) => (
                      <span
                        key={b.id}
                        className="inline-flex items-center gap-1.5 rounded-md bg-white/10 px-2 py-1 text-xs font-semibold text-white/90"
                      >
                        <span className={`h-1.5 w-1.5 rounded-full ${corDoTipo(b.tipo)}`} />
                        <span className="tabular-nums">{b.hora}</span>
                        <span className="font-normal text-white/55">{b.tipo}</span>
                      </span>
                    ))}
                  </div>
      ...
      {escondidos > 0 && (
        <button
          ...
          className="mt-2.5 w-full rounded-lg border border-white/20 bg-slate-950/35 px-4 py-2.5 text-sm font-semibold text-white/85 backdrop-blur-xl active:scale-[0.99]"
        >
